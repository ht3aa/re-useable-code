...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field" :inline-label-vertical-alignment="\Filament\Support\Enums\VerticalAlignment::Center">
    @capture($content)
        <button x-data="{
            ...
            repeaterState: $wire.{{ $applyStateBindingModifiers("\$entangle('{$statePath}'.split('.').slice(0, -2).join('.'))") }},
        
            repeaterStatePath: '{{ $statePath }}'.split('.').slice(0, -2).join('.'),
        
            fieldName: '{{ $fieldName }}',
        
            resetAll: function() {
                Object.keys(this.repeaterState).forEach((statePath) => {
                    $wire.set(this.repeaterStatePath + '.' + statePath + '.' + this.fieldName, false, false);
                })
            },
